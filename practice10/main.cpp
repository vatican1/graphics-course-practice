...
void main()
{
    ...
    vec3 camera_direction = camera_position - position;
    vec3 dir = 2.0 * real_normal * dot(real_normal, camera_direction) - camera_direction;
    ...
    vec3 mean_light = (lightness * albedo + texture(environment_texture, vec2(x, y)).rgb) / 2.0;
    out_color = vec4( mean_light, 1.0);
}
)";
...
int main() try
{
    ...
    GLuint fictive_vao;
    glGenVertexArrays(1, &fictive_vao);
    ...
    while (running)
    {
        ...
        glUniform3fv(env_camera_location, 1, reinterpret_cast<float *>(&camera_position));
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, environment_texture);
        glUniform1i(env_texture_location, 2);
        glDisable(GL_DEPTH_TEST);
        glBindVertexArray(fictive_vao);
